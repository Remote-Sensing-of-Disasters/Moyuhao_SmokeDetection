...
def train(td_path, vd_path, result_path, bands, output_vertex, hidden_vertex, num_layer, echo, batchsize, lr):
    ...
    for e in range(echo):
        ...
        for step,(batchData, batchTarget) in enumerate(trainingDataLoader):
            ...
            #print('训练损失：{}'.format(trainingLoss))
        ...
        if temp < fone_vd:  ##############################注意啊这里被改成看验证数据的f1了
            os.remove(mn)
            mn = 'fcn_{}.pth'.format(e)
            torch.save(net.state_dict(), mn)
            temp = fone_vd
