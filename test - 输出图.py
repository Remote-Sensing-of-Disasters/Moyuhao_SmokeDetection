...
def test_samples(net_file, net_para, test_samples_files, result_pth, name, bands):
    ...
    def output_one_sample(smp_pth, net):
        ...
        if 1:  # 成图代码
            ...
            mk_pred_pic(img, file,
                        'test_{}_{}'.format(smp_pth.split('\\')[-2],smp_pth.split('\\')[-1]))  # 成图代码
            print(TP, TN, FP, FN)
        return TP, TN, FP, FN
    ...
    for i,sample in enumerate(test_samples_files):
        ...
        string = 'Accuracy = {}|Precision = {} | Recall = {}| IoU = {}| F1score = {}| Kappa coefficient={}'.format(
            acc, pre , rcl, iou, fone, kpa)
        print(string)
    accuracy /= i+1
    precision /= i+1
    recall /= i+1
    ...
    f1 /= i+1
    kappa /= i+1
    string = 'In total, Accuracy = {}|Precision = {} | Recall = {}| IoU = {}| F1score = {}| Kappa coefficient={}'.format(accuracy,precision,recall,IoU,f1,kappa)
    print(string)


if __name__ == '__main__':
    result_pth = r'I:\MLP_RESULT\20210201\256_3_BNback'
    net_file = result_pth+r'\fcn_246.pth'
    ...
    #test_samples_files = glob.glob(r'E:\SmokeDetection\source\new_new_data\*\*.tif')
    test_samples_files = glob.glob(r'E:\SmokeDetection\source\test_data\*\0530.tif')
