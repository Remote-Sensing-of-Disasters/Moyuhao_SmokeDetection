...
def test_samples(net_file, net_para, test_samples_files, result_pth, name, bands):
    ...
    def sieve_tvdata(data,smp_pth,net_file):
        ...
        data = data.tolist()
        tvdata = tvdata.tolist()
        ddd = [str(i) for i in data]
        #print(ddd[0])
        #print(time.asctime())
        eee = [str(j) for j in tvdata]
        #print(eee[0])
        #print(time.asctime())
        ...
        #print(ddd[0])
        #print(time.asctime())
        testdata = np.array([eval(p) for p in ddd])
        #print(time.asctime())
        print('已经完成{}_{}图像筛选'.format(date,datetime))
        sadg
    ...
    def output_one_sample(smp_pth, net):
        ...
        Data = ary.reshape([19, ary.shape[1] * ary.shape[2]]).T #这里需要把原始数据变形成shape=【19,291582】再转置
        Data = sieve_tvdata(Data,smp_pth,net_file)
    ...
    with open(result_pth+os.sep+'Evaluation report_fourdays_{}.txt'.format(name),'w') as f:
        f.write(string)
        print('成功输出评价参数结果')
...
net_files = glob.glob(r'E:\SmokeDetection\source\MLP_90_exp\4000_cirrus\1\*f1.pth')
